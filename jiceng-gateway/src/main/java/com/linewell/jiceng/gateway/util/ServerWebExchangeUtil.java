...
import com.linewell.jiceng.gateway.common.FileUploadHttpServletRequest;
import com.linewell.jiceng.gateway.common.RequestContentDataExtractor;
import com.linewell.jiceng.gateway.common.SopServerHttpRequestDecorator;
import com.linewell.jiceng.gateway.param.ApiParam;
import com.linewell.jiceng.gateway.param.FormHttpOutputMessage;
import com.linewell.jiceng.gateway.param.ParamNames;
import com.linewell.jiceng.gateway.route.ForwardInfo;
...
/***
 *  @author wping created on 2021-01-26 16:09 
 */
@Slf4j
public class ServerWebExchangeUtil {
    ...
    public static ServerWebExchange getForwardExchange(ServerWebExchange exchange, ForwardInfo forwardInfo) {
        ...
        ServerHttpRequest newRequest = builder
                .header(ParamNames.HEADER_VERSION_NAME)
    ...
    public static ApiParam getApiParam(ServerWebExchange exchange, byte[] body) {
        ...
        if (StringUtils.containsAny(contentTypeStr, "json", "text")) {
            JSONObject jsonObject = JSON.parseObject(new String(body, JiCengConstants.CHARSET_UTF8));
            ...
            params = RequestUtil.parseQueryToMap(new String(body, JiCengConstants.CHARSET_UTF8));
        }
        if (params != null) {
            apiParam.putAll(params);
        }
    ...
    public static ApiParam getApiParam(ServerWebExchange exchange) {
        return exchange.getAttribute(JiCengConstants.CACHE_API_PARAM);
    }
    ...
    public static void setApiParam(ServerWebExchange exchange, ApiParam apiParam) {
        exchange.getAttributes().put(JiCengConstants.CACHE_API_PARAM, apiParam);
    }
    ...
    public static HttpServletRequest getFileUploadRequest(ServerWebExchange exchange) {
        return exchange.getAttribute(JiCengConstants.CACHE_UPLOAD_REQUEST);
    }

    public static void setFileUploadRequest(ServerWebExchange exchange, HttpServletRequest request) {
        exchange.getAttributes().put(JiCengConstants.CACHE_UPLOAD_REQUEST, request);
    }
