...
import com.linewell.jiceng.gateway.param.ApiUploadContext;
import com.linewell.jiceng.gateway.param.UploadContext;
import lombok.Data;
...
public class RequestUtil {
            ...
            for (Map.Entry<String, ?> entry : apiParam.entrySet()) {
                ...
                if (value instanceof Collection) {
                    ...
                    for (Object el : collection) {
                        list.add(key + "=" + URLEncoder.encode(String.valueOf(el), JiCengConstants.UTF8));
                    }
                } else {
                    list.add(key + "=" + URLEncoder.encode(String.valueOf(value), JiCengConstants.UTF8));
                }
            }
        } catch (UnsupportedEncodingException e) {
            log.error("字符集不支持", e);
    ...
    public static UploadInfo getUploadInfo(HttpServletRequest request) {
        ...
        try {
            ...
            for (FileItem fileItem : fileItems) {
                if (fileItem.isFormField()) {
                    uploadParams.put(fileItem.getFieldName(), fileItem.getString(JiCengConstants.UTF8));
                } else {
